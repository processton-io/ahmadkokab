...
import { useSkillRelatedProjects } from '../../hooks/useSkillRelatedProjects';
...
export default function SkillRelated({ id, ...props }) {
  ...
  const projects = useSkillRelatedProjects();
  const filteredProjects = projects.filter(({ node }) => node.frontmatter.skills.includes(id));

  return (
    <>
        {filteredProjects.length > 0 && <div className="mx-auto container max-w-screen-xl mt-4">
          <div className="mx-auto max-w-screen-xl">
              <h3 className='text-2xl md:text-2xl font-bold text-gray-900 dark:text-white'>Projects using this skill</h3>
          </div>
          <div className="mx-auto grid max-w-screen-xl grid-cols-1 gap-3 md:gap-6 md:grid-cols-3 lg:grid-cols-5 my-8">
